/*
Tarun Shivakumar, Ashwath Kallam, Varun Wanjari
1/10/2023
JavaScript logic for the Minesweeper game, handling cell creation, bomb placement, 
adjacency values, and user interactions using jQuery for the frontend
*/
var idValue;
var cellArray = [];
var difficulty;
...
// Function to set adjacency values which is number of mines that are directly adjacent to a 
//particular cell on the game board for bomb cells
var touch = function() {
  // Get the IDs of bomb cells
  var bId = bombCells();
  // Array to store all adjacent cells
  var allCells = [];

  // Loop through each bomb cell ID
  for (l = 0; l < bId.length; l++) {
    var z = bId[l];
    
    // Calculate IDs of adjacent cells in all directions
    var nw = z - (base + 1);
    var n = z - base;
    var ne = z - (base - 1);
    var w = z - 1;
    ...
    // Push all adjacent cell IDs to the array
    allCells.push(nw);
    allCells.push(n);
    allCells.push(ne);
    allCells.push(w);
    ...
    // Adjust the array based on the position of the bomb cell
    if (z < base) {
  ...
  // Return the array of adjusted adjacent cell IDs
  return allCells;
}
...
// Function to handle clicking on cells to the south
var south = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i += base) {
    // Check if the cell has not been iterated before
    if (cellArray[i].iterator === 0) {
      cellArray[i].iterator += 1;
      clickExpander(i); // Function to handle cell expansion
    }
...
// Function to handle clicking on cells to the north
...
// Function to handle clicking on cells to the east
var east = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i++) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the rightmost column
...
// Function to handle clicking on cells to the west
var west = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i--) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the leftmost column
...
// Function to handle clicking on cells to the northeast
var northeast = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i -= (base-1)) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the rightmost column or top row
...
// Function to handle clicking on cells to the northwest
var northwest = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i -= (base+1)) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the leftmost column or top row
...
// Function to handle clicking on cells to the southeast
var southeast = function(thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i += (base+1)) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the rightmost column or bottom row
...
// Function to handle clicking on cells to the southwest
var southwest = function (thisPlaceholder) {
  for (i = parseInt(thisPlaceholder); i < cellArray.length; i += (base-1)) {
    ...
    if ($("#" + i).hasClass("flag")) {
      ...
      // Check if the cell has no adjacent mines and is on the leftmost column or bottom row
